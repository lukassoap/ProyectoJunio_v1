...
<body>
  <div class="container container-box">
    <h1 class="text-center mb-4"><i class="bi bi-calendar-plus-fill me-2"></i>Crear Cita</h1>
    ...
    <form action="{{ route('citas.store') }}" method="POST">
      ...
      <div class="mb-3">
        ...
        <select name="tramite_id" class="form-select @error('tramite_id') is-invalid @enderror">
          ...
          @foreach($tramites as $t)
            <option value="{{ $t->id }}" {{ old('tramite_id')==$t->id?'selected':'' }}>
              {{ $t->titulo }}
            </option>
          @endforeach
        </select>
      ...
      <div class="d-flex justify-content-between">
        <button class="btn btn-primary"><i class="bi bi-save2-fill me-1"></i>Guardar</button>
